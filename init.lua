...
require('lazy').setup({
  ...
  { -- Autocompletion
    ...
    opts = {
      keymap = {
        ...
        ['<C-Space>'] = false,
        ['<CR>'] = {'accept', 'accept_and_enter', 'select_accept_and_enter', 'select_accept_and_enter'},
  ...
  },
  {
    ...
    config = function(_, opts)
      ...
      if opts.dap and vim.fn.has 'nvim-dap' == 1 then
        local mason_root = vim.fn.stdpath 'data' .. '/mason/packages'

        -- java-debug-adapter jars
        local dbg_root = mason_root .. '/java-debug-adapter'
        if vim.loop.fs_stat(dbg_root) then
          for _, jar in ipairs(vim.fn.glob(dbg_root .. '/extension/server/com.microsoft.java.debug.plugin-*.jar', false, true)) do
            table.insert(bundles, jar)
          end
        end

        -- java-test jars (if enabled)
        if opts.test then
          local test_root = mason_root .. '/java-test'
          if vim.loop.fs_stat(test_root) then
            for _, jar in ipairs(vim.fn.glob(test_root .. '/extension/server/*.jar', false, true)) do
              table.insert(bundles, jar)
            end
          end
        end
      ...
      local function attach_jdtls()
        ...
        local baseconf = {
          ...
          -- capabilities = vim.fn.has 'cmp-nvim-lsp' and require('cmp_nvim_lsp').default_capabilities() or nil,
          capabilities = require('blink.cmp').get_lsp_capabilities(),
        }
        local extra = (type(opts.jdtls) == 'table') and opts.jdtls or {}

        -- Configuration can be augmented and overridden by opts.jdtls
        local config = vim.tbl_deep_extend('force', baseconf, extra)
        ...
        callback = function(args)
          ...
          if client and client.name == 'jdtls' then
            ...
            if vim.fn.has 'mason.nvim' then
              ...
              if opts.dap and vim.fn.has 'nvim-dap' and mason_registry.is_installed 'java-debug-adapter' then
